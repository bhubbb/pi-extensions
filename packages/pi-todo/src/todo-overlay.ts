...
export class TodoOverlay {
  ...
  private renderWidget(theme: Theme, width: number): string[] {
    ...
    // Pure layout function — returns plain-text lines with correct tree chars
    const layout = renderNestedGoalsLayout(overlayState, this.hiddenCompletedTaskIds);
    const goalCounts = selectGoalCounts(overlayState);
    const headingColor = goalCounts.active > 0 ? "accent" : "dim";
    const showIds = selectShowTaskIds(overlayState);

    const lines: string[] = [];

    // Heading: color the glyph and text
    lines.push(truncate(theme.fg(headingColor, layout.heading)));

    // Render each goal group
    for (const group of layout.goalGroups) {
      lines.push(truncate(theme.fg("dim", group.goalLine)));
      for (const taskLine of group.taskLines) {
        lines.push(truncate(taskLine));
      }
    }

    // Track newly completed tasks for auto-hide (separate from layout)
    for (const task of overlayData.tasks) {
      if (
